...
# Sales Revenue Forecasting Engine: BigQuery ML & Looker Studio
...
* **SQL:** Advanced Aggregations & Data Modeling
...
### 1. Data Preparation
Historical data was aggregated into a time-series format (Daily/Weekly) focusing on `close_value` from 'Won' opportunities. Data was cleaned to ensure chronological consistency for the ARIMA model.
...
* **Actual vs. Forecast:** A continuous timeline showing where the business has been and where it is headed.
...
* **Reliability:** By utilizing a 95% confidence interval, the business can plan for a "floor" of $230k weekly revenue even in conservative scenarios.

---

## Repository Structure
* `sql/data_preparation.sql`: Queries used to format raw CRM data for time-series.
* `sql/model_training.sql`: The ARIMA+ model creation and configuration.
* `sql/forecasting_view.sql`: The final SQL View used to power the Looker Studio dashboard.
